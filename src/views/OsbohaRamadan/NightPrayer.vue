                    ...
                    <div class="row">
                        <div class="col-6 col-md-6 col-lg-6" v-for="day in 30" :key="day">
                            <router-link v-if="foundDays(day) !== 0"
                                :to="{ name: 'ramadan.fill-night-prayer', params: { day: foundDays(day) } }"
                                :class="` ${!foundDays(day) ? 'disabled-link' : ''}`">
                                <img :src="imagePath(day)" alt="night_prayer" class="img-fluid" />
...
export default {
    ...
    methods: {
        ...
        imagePath(day) {
            const imageName = this.inDays(day) ? 'night_prayer_on.png' : 'night_prayer_off.png';
